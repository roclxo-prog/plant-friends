...
> 사이트에 사용하는 모든 이미지/일러스트/아이콘의 출처와 라이선스를 추적합니다.
> **원칙**: CC0 또는 무료 상업 이용 가능 자산만 사용. 저작권/출처 불명 자산 금지.
> 실제 파일을 다운로드해 넣을 때마다 "출처 URL", "작가", "저작자표시 필요 여부"를 채워 주세요.
> 마지막 갱신: 2026-06-06

## 표 보는 법
- **저작자표시 필요?**: "아니오" = 크레딧 없이 사용 가능 / "예" = 페이지·푸터에 크레딧 링크 필수.
- **상태**: `준비중` = 아직 실제 파일 없음(플레이스홀더/큐레이션 단계) / `완료` = 파일 확정·기입 완료.

---

## 브랜드 자산 (자체 제작 · 브랜드 소유)

| 자산명 | 파일 | 출처 | 라이선스 | URL | 저작자표시 필요? | 상태 |
|---|---|---|---|---|---|---|
| 로고 | `public/logo.svg` | 자체 제작 | 브랜드 소유 (독점) | — | 아니오 | 완료 |
| 파비콘(SVG) | `public/favicon.svg` | 자체 제작 | 브랜드 소유 (독점) | — | 아니오 | 준비중 |
| 파비콘 32px | `public/favicon-32.png` | 자체 제작 | 브랜드 소유 (독점) | — | 아니오 | 준비중 |
...
> **수집 완료(위키미디어 공용, 전 213종 중 212종 사진 적용 / 오십령옥 1종은 라이선스 확인 불가로 플레이스홀더 유지)**.
> 라이선스 분포: CC BY-SA 약 135 · CC BY 약 34 · 퍼블릭 도메인 24 · CC0 13 등. **대부분 저작자 표시 의무가 있고, CC0·퍼블릭 도메인 37종은 표시 불필요**.
> **전 212종의 정본 저작자 표시(저작자명 + 라이선스명 + 라이선스 링크 + 원본 링크 4요소)는 `credits.html`** 에서 충족하며, 전 페이지 푸터·결과 화면의 "사진 출처" 링크로 접근 가능합니다.
> 아래 표는 대표 12종 예시입니다(전체 목록은 credits.html).
...
> 후보 출처는 `public/illustrations/ILLUSTRATION-SOURCES.md` 참고.
> **주의**: Storyset 무료 플랜은 저작자표시 필수. unDraw/Pixabay는 불필요.

| 자산명 | 파일 | 출처(예정) | 라이선스 | URL | 저작자표시 필요? | 상태 |
|---|---|---|---|---|---|---|
| 히어로 일러스트 | `public/illustrations/hero.svg` | unDraw(1순위)/Storyset/Pixabay | 무료(상업가능) | (확정 시 기입) | unDraw·Pixabay 아니오 / Storyset 예 | 준비중 |
| 빈 결과 일러스트 | `public/illustrations/empty.svg` | unDraw(1순위)/Storyset/Pixabay | 무료(상업가능) | (확정 시 기입) | unDraw·Pixabay 아니오 / Storyset 예 | 준비중 |
| 완료 일러스트 | `public/illustrations/done.svg` | unDraw(1순위)/Storyset/Pixabay | 무료(상업가능) | (확정 시 기입) | unDraw·Pixabay 아니오 / Storyset 예 | 준비중 |
...
- **unDraw**: 오픈 라이선스, 무료 상업 이용, 저작자표시 불필요. https://undraw.co/license
- **Storyset (Freepik)**: 무료 이용 가능하나 **무료 플랜은 저작자표시 필수**. https://storyset.com/terms

> 규칙: Storyset 자산을 쓰면 사이트 푸터에 "Illustrations by Storyset" 크레딧 링크를 넣을 것. 크레딧이 부담되면 unDraw/Pixabay만 사용.
