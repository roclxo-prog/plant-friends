...
## 매칭 알고리즘 v2 (식물 213종 · 질문 7문항)
필수 3문항(빛·물·목적) + 선택 4문항(장소·반려동물·크기·보는재미).
```
score = (light 일치 ?3:0) + (water 허용 ?3:0) + (purpose 포함 ?2:0)
      + (place 일치 ?2:0) + (size 일치 ?2:0) + (interest 포함 ?1:0)
      + (초보 && 난이도'매우 쉬움' ?1:0)
```
- 미응답 차원은 0점(감점 아님) → 필수 3개만으로도 작동(하위호환).
- **반려동물 안전 필터**: pet='yes'면 toxic_to_pets=true 제외(안전종으로 최소 3종 보장).
- 동점: 난이도 쉬운 순 → common(흔함) 순. 최소 3종 보장.
- 상세: core 12종은 정적 `plants/{id}.html`(SEO·긴 가이드), 전 종은 동적 `plant.html?id=`로 표시.
- 상세 설계: `docs/architecture/v2-expansion.md`
